...
async function loadDashboard() {
  ...
  endpoints.forEach((ep) => {
    const badge = document.querySelector(
      `.nav-item[data-section="${ep}"] .nav-badge`,
    );
...
async function loadClientesSelect() {
  const selects = [
    document.getElementById("venda-id-cliente"),
    document.getElementById("pet-id-cliente"),
  ].filter(Boolean);
...
async function loadSectionData(section) {
  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      const badge = document.querySelector(
        `.nav-item[data-section="${section}"] .nav-badge`,
      );
...
function renderPetsRow(p) {
  const clienteSelect = document.getElementById('pet-id-cliente');
  let nomeCliente = `ID: ${p.id_cliente}`;

  if (clienteSelect) {
    const optCliente = Array.from(clienteSelect.options).find(opt => opt.value == p.id_cliente);
    if (optCliente && optCliente.value !== "") {
      nomeCliente = optCliente.textContent;
    }
  }
  ...
  return `<tr>
    ...
    <td><span class="id-badge">${esc(nomeCliente)}</span></td>
  </tr>`;
}
...
function renderProdutosRow(p) {
  const fornecedorSelect = document.getElementById('produto-id-fornecedor');
  let nomeFornecedor = `ID: ${p.id_fornecedor}`; 

  if (fornecedorSelect) {
    const optFornecedor = Array.from(fornecedorSelect.options).find(opt => opt.value == p.id_fornecedor);
    if (optFornecedor && optFornecedor.value !== "") {
      nomeFornecedor = optFornecedor.textContent;
    }
  }
  ...
  return `<tr>
    ...
    <td><span class="id-badge">${esc(nomeFornecedor)}</span></td>
  </tr>`;
}

function renderServicosRow(s) {
  const petSelect = document.getElementById('servico-id-pet');
  let nomePet = `ID: ${s.id_pet}`; 

  if (petSelect) {
    const opcaoEncontrada = Array.from(petSelect.options).find(opt => opt.value == s.id_pet);
    if (opcaoEncontrada && opcaoEncontrada.value !== "") {
      nomePet = opcaoEncontrada.textContent.split(' (')[0]; 
    }
  }
  return `<tr>
    ...
    <td><span class="id-badge">${esc(nomePet)}</span></td>
  </tr>`;
}

function renderVendasRow(v) {
  const produtoSelect = document.getElementById('venda-id-produto');
  const clienteSelect = document.getElementById('venda-id-cliente');
  let nomeProduto = `ID: ${v.id_produto}`;
  let nomeCliente = `ID: ${v.id_cliente}`;

  if (produtoSelect) {
    const optProd = Array.from(produtoSelect.options).find(opt => opt.value == v.id_produto);
    if (optProd && optProd.value !== "") {
      nomeProduto = optProd.textContent.split(' (R$')[0];
    }
  }
  if (clienteSelect) {
    const optCli = Array.from(clienteSelect.options).find(opt => opt.value == v.id_cliente);
    if (optCli && optCli.value !== "") {
      nomeCliente = optCli.textContent;
    }
  }
  ...
  return `<tr>
    ...
    <td><span class="id-badge">${esc(nomeProduto)}</span></td>
    <td><span class="id-badge">${esc(nomeCliente)}</span></td>
  </tr>`;
}
